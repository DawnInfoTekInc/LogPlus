...
public class TextSearchResultResolver extends AbstractResolver {
	...
	public String resolveValue(HttpServletRequest httpRequest, Map<String, Object> parameters) {
		...
		try {		
			...
			String searchMode = LogPlusUtils.getLogProperty(parameters.get("fieldName") + "." + parameters.get("package") + ".search", null);
			
			String text = (String) parameters.get("sourceText");
			
			if(searchMode != null && text != null) {
				
				String startTag = LogPlusUtils.getLogProperty(parameters.get("fieldName") + "." + parameters.get("package") + ".start.tag", null);;
				
				String endTag = LogPlusUtils.getLogProperty(parameters.get("fieldName") + "." + parameters.get("package") + ".end.tag", null);;				
